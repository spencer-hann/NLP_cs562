...
def train_logistic_regression(X, y):
    ...
    lr = LogisticRegression(solver='saga',
                            ...
                            max_iter=4000, # seems to need about 3595
                            verbose=1)
    return lr.fit(X,y)
...
def transform_tf_idf(X_train_counts, X_dev_counts, X_test_counts):
    ...
    X_dev_counts = tf_transformer.fit_transform(X_dev_counts)
    X_test_counts = tf_transformer.fit_transform(X_test_counts)

    return (X_train_counts, X_dev_counts, X_test_counts), tf_transformer
